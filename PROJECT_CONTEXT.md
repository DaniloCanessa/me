...
> Última actualización: 14 de septiembre 2026 (sesión 36, 9–14 sep — **commiteado y desplegado**. (1) **Guarda de precio en el catálogo:** el bloque "Costos internos" —duplicado en el modal de `/admin/products` y en la ficha completa— se extrajo a `components/admin/CostosInternos.tsx`. Cuando `costo × margen` no coincide con el precio guardado, muestra los dos números y obliga a elegir **Conservar $X** / **Actualizar a $Y**; por defecto conserva, salvo que se haya tocado el costo o el margen en esa edición. Editar la cantidad de paneles ya no reescribe el precio en silencio. (2) **Simulador de empresas — planta dimensionable hasta el empalme.** Caso real: *Edificio Jardines de Andalién*, 72 kW de potencia contratada, y el simulador ofrecía solo 39,2 kW porque `buildBusinessKit` dimensiona al **90% del consumo anual** y no había forma de subir. Ahora `businessSizingRange()` (nuevo, en `lib/calculations.ts`) entrega **recomendada / mínima / máxima**, y `buildBusinessKit` acepta `overrideKWp`; en el paso de resultados hay un bloque **"Tamaño de la planta"** con slider (un paso = un panel) y botones *Recomendada* / *Máxima*, más un aviso ámbar de que sobre la recomendada el excedente se paga al 50%. Junto a los kW se muestra la **superficie de techo y los paneles** (`55 m² · 17 paneles de 700 W`), calculados con las medidas reales del panel y actualizados en vivo al mover el slider: al dimensionar lo primero que hay que saber es si el techo da. Verificado en el navegador con el caso real (**1.683 kWh/mes**, que es lo que lee la simulación de Andalién): **11,9 kW** (recomendada) → 55 m² · $3.664.826/año · payback 3 a 2 m · 82% de la cuenta; **71,4 kW** (máxima del empalme) → 332 m² · $13.905.320/año · payback 5 a 1 m · 100%. Seis veces la planta da 3,8 veces el ahorro: esa es la penalidad de inyectar al 50%. (3) 🔴 **El informe de empresa decía "de 550 W cada uno"** en una planta de 39,9 kW: los kits de empresa se arman al vuelo y no traen panel, así que `SimulationReportHtml` caía a la constante del código en vez del valor de `/admin/config`. Ahora la planta de empresa usa el **panel real del catálogo** (el del kit más grande; hoy Astroenergy 700 W) → potencia, cantidad de paneles y m² salen de sus medidas, igual que en residencial. Los m² pasaron de 2,7 m²/panel promedio a los reales (3,25) — el informe subestimaba el techo necesario ~17%. (4) 🔴 **La página 1 del informe se desbordaba en empresa:** la ficha del cliente lleva 2 campos más (potencia contratada y tensión) y el pie quedaba escrito **encima del precio del kit**. Se emparejaron campos cortos (`KVStackPair`): *Tipo de cliente + Teléfono*, *Potencia contratada + Tensión*, *Consumo promedio + anual*. El contenido termina en 1057 px con el pie en 1081 (antes 1119). Verificado en empresa **y** en residencial. (5) **Cabecera del informe sin costura:** la foto arranca en el 38% del ancho y ese borde era un canto duro; el degradado llegaba ahí con 0,73 de opacidad, así que lo atenuaba pero no lo escondía. **La foto se queda donde estaba** (62% derecha, al usuario le gustaba así): lo que cambió es el degradado — **opaco hasta el 40%**, que se come el canto entero, y **9 paradas en vez de 4**, porque con pocas el salto de opacidad se lee como banda vertical al rasterizar con html2canvas. Se probó A/B con una página temporal en `public/` (ya borrada) en vez de rehacer el wizard entero. (6) **Por qué la recomendada sale tan chica (pregunta del usuario):** la recomienda el **consumo**, no el empalme — `kWp = consumo mensual × 12 × 0,9 ÷ producción anual por kWp`, redondeado a paneles enteros. Los 11,9 kW son 17 paneles de 700 W, lo que implica un consumo entre **1.636 y 1.735 kWh/mes** en RM (1.436–1.523 en Biobío). El empalme es techo, no objetivo: el motor no lo llena porque el autoconsumo vale $220/kWh y la inyección $110. **Pendiente del usuario:** validar si esos ~1.683 kWh/mes son correctos para el edificio (podría ser que se cargaran pocas boletas y el resto lo estimara la extrapolación estacional, que se ingresara el monto en $ en vez de kWh —se convierte a $220/kWh y subestima—, o que sea solo el medidor de espacios comunes). **Verificación:** `tsc`, `eslint` y `npm run build` (limpio, desde cero) al cierre. Sin migraciones de base de datos.)
>
> Última actualización: 7 de septiembre 2026 (sesión 35 — **CRM: quién atiende a cada cliente + fix de fusión de fichas**. (1) `assigned_to` sale a la superficie: columna **"Atendido por"** en `/admin/clients` (editable en línea desde el listado, `assignClient`), selector en el modal de alta y en la ficha (`ClientDetail`). Toda ficha nace con vendedor: al crearla a mano toma el del formulario o, si no se tocó, el de la sesión abierta; al convertir un lead se hereda el vendedor del lead; al cotizar desde el simulador queda el usuario que cotiza. Hoy hay un solo usuario, así que no cambia nada visible — deja de dar igual con el primer vendedor que entre. **Sin migración**: `clients.assigned_to` ya existía desde `migration_fase2.sql`. (2) 🔴 **Fix real de datos: el email por sí solo unía a dos personas distintas.** En el back-office es normal poner el correo propio cuando no se tiene el del cliente, y la regla anterior ("mismo email = mismo cliente") colgaba todo lo del tercero de la ficha del dueño del correo — la simulación de **William Sarmiento terminó bajo Danilo Canessa**. Ahora `findClienteExistente` (`lib/db/clients.ts`, único para simulaciones y cotizaciones) exige **email + nombre compatible** (`mismoNombre`: normaliza tildes/espacios y acepta contención, para "William Sarmiento" vs "William Sarmiento Pérez"), y si no calza cae a nombre exacto. Antes esa lógica estaba duplicada en `app/admin/quotes/actions.ts` y `app/admin/simulator/actions.ts`. (3) El informe volvió a mostrar solo los watts del panel (fuera el nombre comercial del modelo). Commits `4610b24`, `b226ffd`, `33debe7`, `749941b` — **todos pusheados y desplegados**.)
>
...
**▶ PARA RETOMAR (sesión 37):**
00a. **Anomalías del catálogo que salieron al mirar los precios reales (sesión 36), además de las de más abajo:** hoy los 14 kits tienen `precio == costo × margen` exacto, o sea el recálculo silencioso ya pasó por todos y **no queda precio original en la base contra el cual comparar** — la lista comercial es la única referencia. `pfv-11kw` y `pfv-13.9kw` son además los **únicos con margen 30%** (el resto va en 10–12%). Y **`pfv-11kw-battery` cuesta solo $541.000 más que `pfv-11kw`**, cuando en los demás kits la batería suma entre $1,26M y $2,78M de costo: está mal cargado casi seguro. `pfv-1.1kw` ($2.246.219) cuesta casi lo mismo que `pfv-2.2kw` ($2.287.815) por 0,7 kW menos — refuerza darlo de baja.
00b. **Simulador de empresas — decisiones que quedaron abiertas:** el selector de tamaño está visible **también en el simulador público**, no solo en el back-office (se puede acotar a `adminMode` con una línea si se prefiere interno). Y `business.cost_per_kwp_clp` sigue en **$1.000.000/kWp plano**: el usuario decidió revisarlo él contra costos reales.
...
0a. ⚠️ **El precio se recalcula en silencio al guardar.** `base_price_clp` sale de `costo_proveedor_clp × (1 + margen_pct/100)` en un campo oculto (`ProductEditForm` y `ProductsManager`), así que editar **cualquier** campo de un kit reescribe su precio. Al ajustar las cantidades de paneles en la sesión 34 se movieron 5 precios sin aviso. Los 4 restantes quedaron así y **conviene validarlos contra la lista real**: `pfv-11kw-battery` 17.700.000 → 13.703.300 · `pfv-3.3kw-battery` 5.460.000 → 6.138.352 · `pfv-13.9kw` 16.680.000 → 16.900.000 · `pfv-11kw` 13.200.000 → 13.000.000. ✅ **Resuelto en la sesión 36:** el bloque "Costos internos" se extrajo a `components/admin/CostosInternos.tsx` (compartido por el modal del listado y la ficha completa, antes duplicado) y ahora, cuando `costo × margen` no coincide con el precio guardado, muestra ambos números y obliga a elegir **Conservar $X** o **Actualizar a $Y**. Por defecto **conserva**, salvo que se haya tocado el costo o el margen en esa edición — que es la única forma de decir "quiero mover el precio". Así, editar la cantidad de paneles ya no reescribe el precio. ⚠️ Recordar que **Supabase es la misma base en local y producción**: cualquier edición de precio queda viva en el sitio de inmediato, sin desplegar.
...
6. **Sección empresas (sesión 36, parcial):** ✅ dimensionado hasta el empalme con selector, panel real y arreglo del desborde del informe. ⚠️ **Queda el precio:** `business.cost_per_kwp_clp` es **$1.000.000/kWp plano**, contra $640.000–$890.000/kWp del catálogo residencial real — una planta de 71,4 kW sale en $71.400.000. El usuario decidió **revisarlo él** contra costos reales de proyectos grandes (sesión 36). Pendiente también el análisis tarifario BT2/BT3/BT4. Nota: el selector de tamaño está visible **también en el simulador público**, no solo en el back-office.
