...
> Última actualización: 21 de junio 2026 (sesión 29 — simulador con lógica de PFV existente [saldo de empalme / complemento / mantención] **desplegado**, commit `9848928`; fotos de proyectos comprimidas, commit `0a9d88b`. Sesión 28 — "Atención a" en clientes + RUT/atención en el PDF de cotización [desplegado])
...
**▶ PARA RETOMAR (sesión 29/30):**
1. **Simulador PFV existente — ✅ DESPLEGADO (sesión 29, `9848928`).** Afinables pendientes si surgen: el umbral "a tope" está fijo en **1 kWp** (`MIN_HEADROOM_KWP` en `StepResults.tsx`); el residual para base "consumo total" es una estimación simple (producción anual de la PFV ÷ 12).
