...
**▶ PARA RETOMAR (sesión 32/33):**
0. 🔴 **CORRER LA MIGRACIÓN `supabase/facturas_v2.sql`** en el SQL Editor de Supabase (es idempotente; al final trae una consulta que lista facturas duplicadas — debe dar 0 filas). Sin ella el módulo `/admin/facturas` no funciona. Después: probar en local el ingreso de un XML real, la clasificación, un pago de venta y la conciliación de un mes ya cargado. **Nada commiteado ni desplegado.**
...
1. ✅ **F29 DESPLEGADO (sesión 31):** los commits del módulo F29 de la sesión 30 (`1876a13`…`a7399bd`) se pushearon a producción junto con el rediseño del simulador (fueron arrastrados por el push de `46c2cec`). **Las 8 migraciones ya están corridas en Supabase** (`sales_invoices`, `f29_periods`, `anticipo_factura_cycle`, `sii_rcv`, `balance_anual`, `gasto_activo_fijo`, `f29_oficial`). **NUEVO PENDIENTE (simulador, revisado con el usuario, no aplicado):** los **correos al cliente** (confirmación de lead en `app/api/leads/route.ts` → `buildClientEmailHtml`, y el del informe en `app/api/send-report/route.ts`) están en **verde y sin logo** → alinearlos al **azul de marca + logo** (opcional: dar más protagonismo al ahorro anual en la confirmación).
